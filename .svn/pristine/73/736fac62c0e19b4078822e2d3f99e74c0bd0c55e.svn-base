...
import com.ymy.xxb.filter.SqlInjectInterceptor;
...
public class CoreFilterChainWebMvcConfigurerAdapter extends  WebMvcConfigurerAdapter{
	...
	public void addInterceptors(InterceptorRegistry registry) {
		...
		// Chain2 : 防止SQL注入拦截器
		setInterceptorPattern(registry.addInterceptor(new SqlInjectInterceptor()), AbstractFilterBean.stringToLists(abstractFilterBean.getSqlInjectFilterIgnoreUrl()));
		// Chain3 : 获取Token，首页 不进行拦截，其他所有请求需要进行accessToken验证
